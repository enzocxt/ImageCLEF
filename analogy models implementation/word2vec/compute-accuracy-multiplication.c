...
int main(int argc, char **argv)
{
  ...
  float dist, len, bestd[N], aresult, bresult, cresult;
  ...
  while (1) {
    ...
    for (c = 0; c < words; c++) {
      ...
      for (a = 0; a < size; a++) aresult += M[a + b1 * size] * M[a + c * size];      // dist is the dividend (v(a)*v(b))
      for (a = 0; a < size; a++) bresult += M[a + b2 * size] * M[a + c * size];   // cosine similarity (a,d)
      for (a = 0; a < size; a++) cresult += M[a + b3 * size] * M[a + c * size];

      aresult = (aresult + 1)/2;
      bresult = (bresult + 1)/2;
      cresult = (cresult + 1)/2;

      if(aresult == 0)  aresult = 0.001;
      dist = (cresult * bresult) / aresult;
