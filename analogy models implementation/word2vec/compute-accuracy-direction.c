...
int main(int argc, char **argv)
{
  ...
  float dist, len, bestd[N], vec[max_size], vecc[max_size];
  ...
  while (1) {
    ...
    if (b == words) continue;       
    for (a = 0; a < size; a++) vec[a] = M[a + b1 * size] - M[a + b2 * size];
    TQS++;
    for (c = 0; c < words; c++) {
      if (c == b1) continue;
      if (c == b2) continue;
      if (c == b3) continue;
      dist = 0;
      for (a = 0; a < size; a++) vecc[a] = M[a + b3 * size] - M[a + c * size];
      for (a = 0; a < size; a++) dist += vec[a] * vecc[a];
      for (a = 0; a < N; a++) {
        if (dist > bestd[a]) {
          for (d = N - 1; d > a; d--) {
            bestd[d] = bestd[d - 1];
