function [BB] = evaluate_direction(W)
...
split_size = 1000; %to avoid memory overflow, could be increased/decreased depending on system and vocab size
...
%cd = zeros(size(W));
%for iii = 1:400000
%cd(iii,:) = cElement - W(iii,:);
%end
%}
cMatrix = repmat(cElement,400000,1);
cd = cMatrix - W;    % cd = c vector - d vector
aElement = aCopy(k,:);
bElement = bCopy(k,:);
result = cd * (aElement' - bElement');  % result is of 400000x1 format
dist(:,k) = result; %cosine similarity if input W has been normalized
end
